...
void VehicleDynamics::update_simulation(double& x, 
                                        ...
                                        double input_acc, 
                                        double dt){
    calculate_dynamics(vx, yaw, input_delta, input_acc);
    integrate_dynamics(x, y, yaw, vx, dt);
}

void VehicleDynamics::calculate_dynamics(double vx, double yaw, double input_delta, double input_acc){
    x_dot_ = vx * std::cos(yaw);
    y_dot_ = vx * std::sin(yaw);
    yaw_dot_ = vx / kWheelBase * std::atan(input_delta);
    vx_dot_ = calculate_fx(vx, input_acc) / kMass;
}
...
double VehicleDynamics::calculate_fx(double vx, double input_acc){
