...
class KalmanFilter
{
    ...
    public:
        ...
        /**
         * @brief Period initializer
         * Initializes dt and sets "is_periodic_" to true.
         * 
         * @param dt Estimation period
         */
        void set_estimation_period(double dt){
            is_periodic_ = true;
            dt_ = dt;
        }
        ...
        /**
         * @brief Observation matrix updater
         * Updates observation matrix H for this iteration.
         * 
         * @param H Observation matrix
         */
        void update_observation_matrix(MatrixXd H){
            if(H.rows() != p_ || H.cols() != n_) {
                std::cerr << "Observation matrix dimensions are wrong!\np= " << p_ <<", n = " << n_ << std::endl;
            }
            else {
        ...
        void estimate_state(VectorXd u, VectorXd z){
            ...
            if(!is_periodic_){
                ...
                dt_ = std::chrono::duration_cast<std::chrono::nanoseconds>(now - t_).count() * pow(10, -9);
                t_ = now;
            }

            MatrixXd A(n_, n_);
            A = MatrixXd::Identity(n_, n_) + dt_ * M_; 

            // State prediction
            VectorXd x_pred = A * x_ + dt_ * B_ * u_;
