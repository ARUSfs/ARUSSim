...
void calculate_tire_loads(ControllerSim& controller_sim){
    ...
    controller_sim.tire_loads_.rl_ = controller_sim.get_kStaticLoadFront() - controller_sim.get_kMassDistributionRear() * load_transfer_ay + load_transfer_ax/2;
    controller_sim.tire_loads_.rr_ = controller_sim.get_kStaticLoadFront() + controller_sim.get_kMassDistributionRear() * load_transfer_ay + load_transfer_ax/2;
...
VectorXd calculate_steering(ControllerSim& controller_sim){
    ...
    if(controller_sim.delta_ > 0.0f){
        delta_ack(0) = controller_sim.get_kAckermann()*(delta_in_ackermann - controller_sim.delta_) + controller_sim.delta_;
        delta_ack(1) = controller_sim.delta_;
    } else {
        delta_ack(0) = controller_sim.delta_;
        delta_ack(1) = controller_sim.get_kAckermann()*(delta_in_ackermann - controller_sim.delta_) + controller_sim.delta_;
    }
...
void calculate_slip_ratio(ControllerSim& controller_sim){
    ...
    double alpha_fr{0.0}, alpha_fl{0.0}, alpha_rl{0.0}, alpha_rr{0.0};

    if(controller_sim.vx_ > 1){
        alpha_fl = std::atan(vy_front / vx_left) - controller_sim.delta_;
        alpha_fr = std::atan(vy_front / vx_right) - controller_sim.delta_;
        alpha_rl = std::atan(vy_rear / vx_left);
        alpha_rr = std::atan(vy_rear / vx_right);
    }

    double vx_fl = std::sqrt(vy_front*vy_front + vx_left*vx_left) * std::cos(alpha_fl);
    double vx_fr = std::sqrt(vy_front*vy_front + vx_right*vx_right) * std::cos(alpha_fr);
    double vx_rl = std::sqrt(vy_rear*vy_rear + vx_left*vx_left) * std::cos(alpha_rl);
    double vx_rr = std::sqrt(vy_rear*vy_rear + vx_right*vx_right) * std::cos(alpha_rr);
